...
*** SEED WAS SET PRIOR TO BEGINNING 
...
doctors make recommendations about how their patients improve or have more healthier days 
...
gap_stat <-
  clusGap(df_part2, FUN = hclusCut, K.max = 10, B = 50)
...
clusters is 3. Now, we'll use the number of clusters from the gap statistic to 
obtain cluster assignment for each observation in the dataset. 

```{r}
clusters_h3<-cutree(clusters_fit, k=3)
...
ii) The composition of each cluster in terms of the original input features 


5. One research question that can be addressed using the newly identified clusters, 
is "Does living in a state included in each individual cluster predict the number of people 
that would be arrested for a felony crime (murder, assault, rape)?" 
